 ...
 * 
 */
public class HyBRFloatIterativeDeconvolver2D extends AbstractFloatIterativeDeconvolver2D {
    ...
    /**
     * Creates a new instance of HyBRFloatIterativeDeconvolver2D
     * 
     * @param imB
     *            blurred image
     * @param imPSF
                ...
                case TIKHONOV:
                    ...
                    if (k > 1) {
                        if (Math.abs((GCV.getQuick(k - 1) - GCV.getQuick(k - 2))) / GCV.get(begReg - 2) < flatTol) {
                            x = V.zMult(f, null);
                            if (useThreshold) {
                                FloatCommon2D.assignPixelsToProcessor(ip, x, cmY, threshold);
                            } else {
                                FloatCommon2D.assignPixelsToProcessor(ip, x, cmY);
                            }
                            if (logConvergence) {
    ...
    private class FloatSimpleLBD implements FloatLBD {
        ...
        private final FloatPSFMatrix2D A;
    ...
    private class FloatPLBD implements FloatLBD {
        ...
        private final FloatPreconditioner2D P;
